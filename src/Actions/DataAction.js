...
export const getInitials = (fetchAllData) => {
  const nameParts = fetchAllData.split(" ").filter(Boolean); // Ensure no empty parts are included
  const firstInitial = nameParts[0] ? nameParts[0][0].toUpperCase() : "";
  const lastInitial = nameParts[1] ? nameParts[1][0].toUpperCase() : "";
  
  // In case there's only one part of the name, we return only one initial
